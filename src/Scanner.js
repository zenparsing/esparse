...
export class Scanner {
    ...
    skip() {
        ...
        do { token = (this.start = this.offset) >= this.length ? this.EOF() : this.Skip(); }
        while (token === null)
    ...
    next(context) {
        ...
        do { token = (this.start = this.offset) >= this.length ? this.EOF() : this.Start(context); }
        while (token === null)
    ...
    Skip() {
        ...
        return new AST.Token("Unknown", this.start, this.offset);
    }
    ...
    PunctuatorChar() {
        ...
        return new AST.Token(this.readChar(), this.start, this.offset);
    }
    ...
    Punctuator() {
        ...
        return new AST.Token(op, this.start, this.offset);
    }
    ...
    Template() {
        ...
        return new AST.Template(val, end, this.start, this.offset);
    }
    ...
    String() {
        ...
        return new AST.String(val, this.start, this.offset);
    }
    ...
    RegularExpression() {
        ...
        return new AST.RegularExpression(val, flags, this.start, this.offset);
    }
    ...
    LegacyOctalNumber() {
        ...
        if (!isNumberFollow(this.peekChar()))
            return this.Error();
        
        var val = parseInt(this.input.slice(start, this.offset), 8);
        
        return new AST.Number(val, this.start, this.offset);
    }
    ...
    Identifier(context) {
        ...
        if (context !== "name" && reservedWord.test(id))
            return new AST.Token(id, this.start, this.offset);
        
        return new AST.Identifier(id, "", this.start, this.offset);
    }
    ...
    LineComment() {
        ...
        var val = this.input.slice(start, this.offset);
        
        return new AST.Comment(val, this.start, this.offset);
    }
    ...
    BlockComment() {
        ...
        var val = this.input.slice(start, this.offset - 2);
    ...
    EOF() {
        ...
        return new AST.Token("EOF", this.offset, this.offset);
    }
    
    Error() {
    
        return new AST.Token("ILLEGAL", this.start, this.offset++);
    }
